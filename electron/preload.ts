...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  pg: {
    ...
    sessionOpen:    (id: string, database?: string, settings?: Record<string, string>, readOnly?: boolean, autocommit?: boolean) => ipcRenderer.invoke('pg:session-open', { id, database, settings, readOnly, autocommit }),
  ...
  mysql: {
    ...
    sessionOpen:  (id: string, database?: string, opts?: { autocommit?: boolean; consistentSnapshot?: boolean }) => ipcRenderer.invoke('mysql:session-open', { id, database, ...opts }),
  ...
  mongodb: {
    ...
    exportCollections: (id: string, database: string) => ipcRenderer.invoke('mongodb:export-collections', { id, database }),
    exportOpen:   (id: string, database: string, collection: string) => ipcRenderer.invoke('mongodb:export-open', { id, database, collection }),
    exportRead:   (cursorId: string, limit: number) => ipcRenderer.invoke('mongodb:export-read', { cursorId, limit }),
    exportClose:  (cursorId: string) => ipcRenderer.invoke('mongodb:export-close', { cursorId }),
    importPrepare: (id: string, database: string, metadata: string, drop: boolean) => ipcRenderer.invoke('mongodb:import-prepare', { id, database, metadata, drop }),
    importDocuments: (id: string, database: string, collection: string, documents: string[]) => ipcRenderer.invoke('mongodb:import-documents', { id, database, collection, documents }),
  ...
  files: {
    ...
    openDialog:   (opts: { title?: string; filters?: { name: string; extensions: string[] }[]; directory?: boolean; multiple?: boolean }) => ipcRenderer.invoke('file:open-dialog', opts),
    openRead:     (filePath: string) => ipcRenderer.invoke('file:open-read', { filePath }),
    read:         (fileId: string, maxBytes?: number) => ipcRenderer.invoke('file:read', { fileId, maxBytes }),
    closeRead:    (fileId: string) => ipcRenderer.invoke('file:close-read', { fileId }),
